...
func (t *terminal) consumeANSIEscape(buf *bufio.Reader, ansiBuf *bytes.Buffer) (result readResult, err error) {
	...
	case 'u':
		if initial == '[' {
			if keyCode, modifier, ok := parseCSIuPayload(ansiBuf.Bytes()); ok {
				// Terminals with modifyOtherKeys may emit Ctrl+Enter as CSI-u,
				// e.g. ESC [ 13 ; 5 u. Map it to a synthetic key so callers can
				// distinguish it from plain Enter.
				if (keyCode == 10 || keyCode == 13) && modifierHasCtrl(modifier) {
					r = CharCtrlEnter
				}
			}
		}
...
func parseCSIuPayload(payload []byte) (keyCode int, modifier int, ok bool) {
	if len(payload) == 0 {
		return 0, 0, false
	}
	parts := bytes.SplitN(payload, []byte(";"), 2)
	keyCode, err := strconv.Atoi(string(parts[0]))
	if err != nil {
		return 0, 0, false
	}
	if len(parts) == 1 {
		return keyCode, 0, true
	}
	modifier, err = strconv.Atoi(string(parts[1]))
	if err != nil {
		return 0, 0, false
	}
	return keyCode, modifier, true
}

func modifierHasCtrl(modifier int) bool {
	// XTerm modifier encoding is 1 + bitmask:
	// Shift=1, Alt=2, Ctrl=4, Meta=8.
	// Therefore Ctrl is set when (modifier-1)&4 != 0.
	if modifier <= 1 {
		return false
	}
